...
export function AssistantMessage({
  ...
  // Check for TTS output in the stream values
  const ttsOutput = thread.values?.ttsOutput;
  const hasTTSOutput = ttsOutput && 
    ttsOutput.audioData && 
    ttsOutput.audioData.length > 0 &&
    ttsOutput.mimeType &&
    isLastMessage &&
    !isLoading; // Only show when not loading to avoid premature display

  // Debug logging for TTS output
  React.useEffect(() => {
    if (isLastMessage && ttsOutput) {
      console.log("🔊 TTS Output Debug:", {
        hasAudioData: !!ttsOutput.audioData,
  ...
  }, [ttsOutput, isLastMessage, isLoading, hasTTSOutput]);
